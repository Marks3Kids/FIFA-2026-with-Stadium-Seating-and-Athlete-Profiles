  ...
  const handleRestore = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.valid && data.tier && data.tier !== 'free') {
        ...
        setRestoreError(t("landing.restoreModal.noPurchase"));
      }
    } catch {
      setRestoreError(t("landing.restoreModal.connectionError"));
    } finally {
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-card border border-white/10 rounded-2xl p-7 max-w-sm w-full shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-white">{t("landing.restoreModal.title")}</h3>
              <button onClick={() => { setShowRestore(false); setRestoreError(""); }} className="text-muted-foreground hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-sm text-muted-foreground mb-5">
              {t("landing.restoreModal.description")}
            </p>
            <form onSubmit={handleRestore} className="space-y-3">
              <input
                type="email"
                placeholder={t("landing.restoreModal.emailPlaceholder")}
                value={restoreEmail}
              ...
              >
                {isRestoring ? <><Loader2 className="w-4 h-4 animate-spin" /> {t("landing.restoreModal.checking")}</> : <><CheckCircle className="w-4 h-4" /> {t("landing.restoreModal.cta")}</>}
              </button>
            </form>
            <p className="text-xs text-muted-foreground mt-4 text-center">
              {t("landing.restoreModal.problemsEmail")} <a href="mailto:support@championshipconcierge.com" className="text-primary">support@championshipconcierge.com</a>
            </p>
          </div>
        </div>
      )}
          ...
          <div className="flex items-center gap-2">
            ...
            {!isSubscribed && (
              ...
              >
                ...
                <span className="hidden xs:inline">{t("landing.banner.restore")}</span>
            ...
            <div ref={langRef} className="relative">
              ...
              {langOpen && (
                <div className="absolute end-0 top-full mt-1 bg-[#1a2a1a] border border-white/20 rounded-xl shadow-2xl overflow-hidden z-50 min-w-[90px]">
                  {LANGUAGES.map(lang => (
                    <button
                      key={lang.code}
            ...
            <div ref={menuRef} className="relative sm:hidden">
              ...
              {menuOpen && (
                <div className="absolute end-0 top-full mt-2 bg-[#1a2a1a] border border-white/20 rounded-2xl shadow-2xl overflow-hidden z-50 min-w-[220px]">
                  {isSubscribed ? (
                    <Link
                      href="/home"
                    ...
                    <>
                      ...
                      >
                        <Tag className="w-4 h-4" /> {t("landing.getStarted")}
                      </button>
                      <Link
                        href="/home"
                        onClick={() => setMenuOpen(false)}
                      ...
                      >
                        <Globe className="w-4 h-4" /> {t("landing.exploreApp")}
                      </Link>
                    </>
                  )}
                  <button
                  ...
                  >
                    <RefreshCw className="w-4 h-4" /> {t("landing.banner.restoreShort")}
                  </button>
                </div>
              )}
            </div>
      ...
      {!isSubscribed && (
        <div className="pt-16 bg-background">
          <div className="w-full px-4 sm:px-6 py-2.5 flex items-center justify-between gap-3 border-b border-emerald-500/20 bg-emerald-500/5">
            <p className="text-xs sm:text-sm text-emerald-300 font-medium leading-tight">
              <span className="font-bold text-emerald-400">{t("landing.banner.alreadyPurchased")}</span>{" "}
              <span className="hidden sm:inline">{t("landing.banner.accessSavedLong")}</span>
              <span className="sm:hidden">{t("landing.banner.accessSavedShort")}</span>
            </p>
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                ...
                {t("landing.banner.restoreAccess")}
              </button>
              <a
                href="/restore-access"
      ...
      >
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <div className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-4 sm:px-5 py-2 sm:py-2.5 rounded-full mb-4 shadow-[0_0_30px_rgba(34,197,94,0.5)]">
            ...
            <span className="text-xs sm:text-sm font-bold uppercase tracking-wide">{t("landing.oneTimeBadge")}</span>
          </div>
          ...
          <div className="flex justify-center mb-6">
            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full">
              ...
              <span className="text-xs sm:text-sm text-primary font-medium">{t("landing.tournamentDates")}</span>
            </div>
          </div>
          ...
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-display font-bold mb-6 leading-tight">
            ...
            <br />
            <span className="text-amber-400">{t("landing.heroTitle3")}</span>
          </h1>
            ...
            {isSubscribed ? (
              ...
              <>
                ...
                >
                  ...
                  {t("landing.alreadyPurchasedRestore")}
                </button>
              </>
            )}
          </div>
          ...
          <div className="flex items-center justify-center gap-4 sm:gap-8 text-muted-foreground">
            {[
              { value: "48", label: t("landing.teams") },
              { value: "16", label: t("landing.cities") },
              { value: "104", label: t("landing.matches") },
              { value: "5", label: t("landing.languages") },
            ].map((stat, i, arr) => (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {FEATURE_ICONS.map((icon, index) => (
              ...
              >
                ...
                <h3 className="text-base sm:text-lg font-bold text-white mb-2">{t(`landing.feature${index + 1}Title`)}</h3>
                <p className="text-muted-foreground text-sm">{t(`landing.feature${index + 1}Desc`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="py-10 sm:py-12 px-4 border-t border-white/10">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mb-8 sm:mb-10">
            ...
            <div>
              <h4 className="text-white font-semibold text-sm mb-3">{t("landing.footer.quickLinks")}</h4>
              <ul className="space-y-2 text-xs text-muted-foreground">
                <li><Link href="/home" className="hover:text-white transition-colors">{t("landing.footer.exploreApp")}</Link></li>
                <li><Link href="/cities" className="hover:text-white transition-colors">{t("landing.footer.cities")}</Link></li>
                <li><Link href="/support" className="hover:text-white transition-colors">{t("landing.footer.support")}</Link></li>
              </ul>
            </div>

            <div>
              <h4 className="text-white font-semibold text-sm mb-3">{t("landing.footer.legal")}</h4>
              <ul className="space-y-2 text-xs text-muted-foreground">
                ...
                <li><Link href="/policy" className="hover:text-white transition-colors">{t("landing.footer.refund")}</Link></li>
              </ul>
            </div>
            ...
            <div>
              ...
              >
                ...
                <span className="text-xs font-medium">{t("landing.footer.googlePlay")}</span>
              </a>
            </div>
          </div>
